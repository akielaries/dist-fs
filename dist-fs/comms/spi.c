...
static int spi_read(comm_context_t *ctx, uint8_t *rx, uint32_t rx_sz, uint16_t timeout_ms) {
    if (!ctx || !rx) return -EINVAL;
...
static int spi_write(comm_context_t *ctx, uint8_t *tx, uint32_t tx_size, uint16_t timeout_ms) {
    if (!ctx || !tx) return -EINVAL;
